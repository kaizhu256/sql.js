...
exports.test = function (SQL, assert) {
    ...
    function canHandle(testData) {
        ...
        res = db.exec("SELECT CheckTestValue(" + sql_value + ")==12345");
        if (res[0]["values"][0][0] !== 1) {
            if (verbose) assert.ok(false, "Can accept " + testData.info);
            ok = false;
        }
        // Check js to sqlite value conversion
        res = db.exec("SELECT GetTestValue()");
        if (!value_equal(res[0]["values"][0][0], testData.value)) {
            if (verbose) assert.ok(false, "Can return " + testData.info);
            ok = false;
        }
        ...
        if (sql_value !== "null") {
            ...
            res = db.exec("SELECT identityFunction(" + sql_value + ") is null");
        }
        if (res[0]["values"][0][0] !== 1) {
            if (verbose) assert.ok(false, "Can pass " + testData.info);
            ok = false;
        }
        return ok;
    }
    ...
    assert.throws(
        ...
        /internal exception/,
        "Can handle internal exceptions"
    );
